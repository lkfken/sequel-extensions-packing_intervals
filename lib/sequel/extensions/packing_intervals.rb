...
module Sequel
  module Extensions
    module PackingIntervals
      def packing_intervals(partition: nil, dataset: self, start_date: :start_date, end_date: :end_date, cte_alias: :cte, order: nil)
        order = order.nil? ? [:start_date, :end_date] : [start_date, end_date]
        ...
        lag_end_date    = Sequel.function(:lag, end_date).over(:partition => partition, :order => order)
        ...
        reduced = db["#{cte_alias}4".to_sym].
            with("#{cte_alias}1".to_sym, dataset.cross_apply(db["VALUES (1, #{start_date.to_s}), (-1, #{end_date.to_s})"].as(:a, [:type, :ts])).
                ...
                select_append(
                    Sequel.case({-1 => nil}, Sequel.function(:row_number).over(:partition => [partition, :type].flatten, :order => order), :type).as(:s),
                    Sequel.case({1 => nil}, Sequel.function(:row_number).over(:partition => [partition, :type].flatten, :order => order), :type).as(:e))).
        ...
        db["#{cte_alias}7".to_sym].
            ...
            with("#{cte_alias}6".to_sym, db["#{cte_alias}5".to_sym].select_append(Sequel.function(:sum, :grp_start).over(partition: partition, order: order).as(:grp))).
